...
// Tentar default import para haversine, se falhar, voltar para require
import haversine from "haversine-distance";

// Inicialização do Firebase Admin
if (!admin.apps.length) {
  admin.initializeApp({
    // Usamos 'as any' aqui porque o tipo exato do serviceAccount pode variar
...
// --- Tipagem para Ecoponto (melhora a segurança) ---
interface EcopontoData {
    id?: string; // ID opcional pois vem do Firestore
    title: string;
    latitude: number;
    longitude: number;
    materials: string[];
...
app.get("/ecopontos", async (req: express.Request,
  ...
  try {
    ...
    // MUDANÇA 3: Usar o tipo EcopontoData
...
app.get("/ecopontos/perto", async (req: express.Request,
  ...
  try {
    ...
    // MUDANÇA 3: Usar o tipo EcopontoComDistancia
